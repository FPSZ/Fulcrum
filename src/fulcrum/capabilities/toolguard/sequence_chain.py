...
攻击:最典型的「敏感数据读取 → 对外发送」数据外泄链。本分析器在会话轨迹(session_trace)
...
窗口暂以"最近 N 步"近似(ToolIntent 尚无时间戳),N 见 `_WINDOW`;后续接入时间窗即可平滑替换。
...
class SequenceChainAnalyzer:
    ...
    def analyze(self, session_trace: list[ToolIntent], ctx: Context) -> list[Finding]:
        if not session_trace:
            return []
        current = session_trace[-1]
        if not _is_outbound(current):
            return []  # 仅在"对外发送"这步收口判链

        window = session_trace[-(_WINDOW + 1) : -1]  # 当前调用之前、窗口内的历史调用
