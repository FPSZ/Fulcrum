...
> 本文回答什么问题：如何把单组织全局角色演进为组织、团队和范围化角色，并确保跨团队访问受到后端约束。
> 状态：候选重构方案（2026-06，尚未作为当前主基线承诺）；已落地权限能力见 [`../arch/06-用户管理与权限.md`](../arch/06-用户管理与权限.md)，开放 PR 状态见 [`../review/01-开放PR状态与合并门槛.md`](../review/01-开放PR状态与合并门槛.md)。
...
- **P1 · 团队范围化**:`teams`(升格 departments)+ `team_memberships` 多对多 + 组织/团队角色二分;
  ...
  - **P1c(助手)✅ 平权已落(原遗留已清)**:助手以 `Principal` 行事、`visible_for` 按**已范围化**
    权限过滤工具。team_scoped 写工具(改成员状态/资料/口令、增减团队成员、审批/驳回账号)对团队
    负责人放开可见性,执行点(actuator.confirm/undo 用 `visible_to`;handler 用 `_member_in_scope`/
    `can_manage_team`)钉死目标范围:负责人只管本团队子树、不得改派角色/移人出队、审批只能审进本团队
    且只赋团队级角色。组织管理员跨团队管全部。12 条平权回归。
  - **P1b·余 ✅**:角色 org/team scope 二分(scope 列)已落(`16b989f`);前端角色页按范围分组 + 范围
    徽标 + 新建时选范围。
- **P2 · 资源租户化 ✅(事件维度 + 网关归属)**:事件按团队隔离已打通——
  · `screen_input/output/tool_return` 可带 `team_id`,落进判定点 evidence;
  · 网关配置(`GatewayConfig.team_id`)= 被保护智能体归属团队,`/gateway/chat` 据此给其流量打标;
    **前端网关接入表单已暴露"归属团队"选择**;
  · 事件流 `event_team_visible` 行级过滤:**无归属事件(演示/内部)对所有 events.view 可见**(不破坏演示);
    **有归属事件仅本团队成员/负责人 + 组织管理员可见**,跨团队隔离。4 条隔离回归。
  - **遗留(单列大史诗,需产品定调)**:策略/工具等其余资源的租户化、网关**多上游**(一平台多被保护
    系统各属不同团队、按路由键分流)——属新增功能面,语义未定(每团队独立策略文件?多上游路由键?),
    留待主程产品决策后单独评审,不在本轮范围内盲做。
- **P3 · 高级 ✅(本轮收尾)**:
  · **审批下放**:`require_approver` + `_guard_approve/_guard_reject`,团队负责人可审批/驳回"加入本
    团队"的账号(只能审进本团队、只赋团队级角色),超管不再是唯一审批人;8 条回归。
  · **助手平权**:见上 P1c。
  · **嵌套团队继承**:`managed_teams` 已按子树展开(选父团队覆盖其下全部),负责人管整棵子树。
  · **前端范围化**:`/auth/me` 下发 `team_ids/managed_teams`;`require_member_reader` + 成员列表对
    纯负责人行级过滤;module `leadVisible` 让纯负责人也能进"组织与成员"页只管本团队。
  · 自定义团队角色:权限矩阵(无/只读/读写三态)即细粒度增删,叠加 org/team 范围二分,已可由管理员
    自由组合(GitHub 自定义角色等价能力)。
- **前端 IA(§10)✅**:`console/src/features/admin/` 由"单页多 Tab + 三栏"改为**左侧分区导航 + 实体页**
  (成员 People / 团队 Teams / 角色 Roles / 待审批),组织级与团队级在 IA 上分开;typecheck + vite build 通过。
